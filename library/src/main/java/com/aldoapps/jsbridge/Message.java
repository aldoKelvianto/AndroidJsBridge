    ...
    public static List<Message> toArrayList(String jsonStr) {
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                message.setHandlerName(jsonObject.has(HANDLER_NAME_STR) ? jsonObject
                    .getString(HANDLER_NAME_STR) : null);
                message.setCallbackId(
                    jsonObject.has(CALLBACK_ID_STR) ? jsonObject.getString(CALLBACK_ID_STR) : null);
                message.setResponseData(jsonObject.has(RESPONSE_DATA_STR) ? jsonObject
